...
- 3D 种子图是切 3D 时的派生产物：客户端弹出 3D 种子确认向导，先以全身种子图为参考生成 A-pose、3D 画风的正面种子；供应商支持多视角时再从 3D 正面种子派生背面种子（预览、微调或重新生成、历史画廊），确认后按双视图（front + back）提交，背面生成失败可降级为仅提交正面图。不支持多视角的供应商只有 3D 正面一步，确认即提交。
...
- 双足姿态随画风路由：2D 立绘画风（refined_anime_cg）自然站姿（see-through 拆分不要求 A-pose）；3D 画风（anime_game_cg / realistic）A-pose（绑骨识别与多视角一致性）。非双足骨架（四足/鸟/蛇等）姿态本为自然站/舒展，不随画风切换。
