...
- 头像种子图锚定角色身份。独立的全身种子图在头像确认后生成，以头像与角色定义补全身材比例，通过自然优美的姿态展现整体气质；不要求固定站姿，不使用建模姿态模板，也不进入 2D / 3D 模型生成链。每次重绘仍从头像出发，可附带用户参考图作为第二参考来补充体型、服装和姿态，身份以头像和角色定义为准；不用上一版全身图递归派生。
- 房间背景、聊天画/拍自己及夜间出镜媒体使用独立全身种子图，文件缺失或不可读时停止生成并提示重新生成种子图。房间生成只传这一张图锚定身份与身材；当前服装、发型和配饰来自衣柜激活外观的着装描述，优先于参考图中的穿着。夜间出镜图片可将激活外观作为第二参考来确定穿着，脸部和身材比例由全身种子图锚定。出镜视频的缺省首帧使用全身种子图。
- 引导流程在全身种子图之后生成并确认 2D 正面种子图（自然站姿、赛璐珞画风）——它是 2D 分层动画的唯一必需输入与 2D 重切分的资产源，其身份参考源自头像或用户提供的身份参考图，确认后不再变更。当前外观的姿态资产以本次立绘为基准，闭眼附件以对应姿态为基准。
- 3D 种子图是切 3D 时的派生产物：客户端弹出 3D 种子确认向导，先以头像为参考生成 A-pose、3D 画风的正面种子；供应商支持多视角时再从 3D 正面种子派生背面种子（预览、微调重绘、历史画廊），确认后按双视图（front + back）提交，背面生成失败可降级为仅提交正面图。不支持多视角的供应商只有 3D 正面一步，确认即提交。
