...
- 引导流程在全身种子图之后生成并确认 2D 正面种子图（自然站姿、赛璐珞画风）——它是 2D 分层动画的唯一必需输入与 2D 重切分的资产源，重新生成的身份与身材参考恒源自独立全身种子图（不接受用户参考图直传），微调直接编辑上一版正面种子，确认后不再变更。当前外观的姿态资产以本次立绘为基准，闭眼附件以对应姿态为基准。
...
- 衣柜换装外观草稿重新生成时恒以独立全身种子图为主参考图（图 1 身份与身材锚点，保留头像身份且不丢身材比例），用户附带的服装图为次参考图（图 2 仅供提取服装/发型样式）；微调则直接编辑上一版草稿立绘。坚决不使用已有 2D 立绘作为重新生成的参考，避免画风串染与迭代失真。
