...
姿态提示词按参考职责、姿势与神态、完整构图和渲染要求组织；角色设计取自本次立绘，关节姿势取自人体参考，最终成图独立满足完整轮廓与留白要求。视觉模型按身份、服饰、风格、轮廓及肢体连通性验收，几何关系由坐标校验负责；闭眼编辑限定眼部像素，视觉模型验收闭合状态与面部结构，配准由像素校验负责。重试沿用完整成图要求，视觉反馈以目标描述输出，并与几何修正目标一样作为独立 JSON 数据传入。
