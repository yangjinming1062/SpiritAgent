...
已切分成功的外观可单侧重生成一侧扶边姿态（`POST /api/companion/outfits/{id}/poses/{left|right}/regenerate`，仅 `ready` 状态）：输入仍为该外观立绘原图，只替换该侧两张姿态纹理与描述符 `poses` 子树并重算整包哈希，另一侧与分层 PSD 不动；与整包切分互斥，同一外观同一时间只允许一个单侧任务。生成失败保留旧姿态、外观保持 ready，不波及整套资产；在飞任务不落库，进程重启即丢，由客户端兜底超时清理提示。
