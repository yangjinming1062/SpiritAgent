...
所有路径首个供应商流事件后禁止切换供应商。客户端气泡收尾须对 TTS 幂等：若尾部分隔事件已提交最后一个气泡，随后完成事件仅补媒体、消息 id 等元数据，保留播放状态，不再次合成。
