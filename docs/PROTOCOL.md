...
引导恢复依据服务端状态及已有资产，不因页面重开重复生成。角色定稿不等于引导完成；全身参考、外观参考立绘确认和音色完成后才结束，用户资料可选。草稿加载失败只重试加载，过期才重新生成。

外观参考立绘确认后锁定头像、物种、性别与基础外貌；全身参考重绘、换装、种子派生和模型生成不解锁身份。确认只表示参考图就绪，不代表模型或视频等可播放形象已就绪。编辑、重生成与自备图的统一规则只在 [PIPELINE](PIPELINE.md#11-共用参考与种子图派生) 定义，各入口不得形成不同语义。
...
**回复与偏好。** 陪伴回复采用[结构化气泡](../backend/modules/conversation/replies.py)，由模型逐泡选择文字或语音；仅语音携带 `speech` 演绎，按实际 MiMo / MiniMax 能力校验。`companion.response_preference` 保存偏好，`prompt.submit.response_preference` 携带本轮快照；客户端不据此转换消息或自动播放。
...
主动回合使用同一结构，`{"bubbles":[]}` 表示沉默、不写空消息，常规档仅提供文字能力。音频准备后重新检查用户活动、租约和档位；接受时原子保存消息与 `companion.message`，拒绝或取消清理未交付音频。

供应商流收到首个事件或非流式响应返回后，不再切换供应商。未交付正文、非内容过滤的不完整终态最多允许同供应商同参数重试一次，不复用半截正文或工具草稿，不重跑已完成工具。模型取消或异常不交付未确认正文；失败的模型回合不写终端正文行。
