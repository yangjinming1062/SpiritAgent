...
片刻由伙伴创建，用户只能评论、删除本人评论或隐藏整条；日记允许用户编辑，后台只能追加已编辑内容。生活空间工具只向陪伴预设开放，装配与派发两层均校验，不能仅隐藏界面入口。
...
历史消息必须携带持久化 ID 与毫秒级创建时间。全量恢复达到防御上限时返回截断标记和分页游标；`GET /api/sessions/{id}/messages?before_id=...&limit=...` 返回紧邻的更早一页，页内 ID 升序，`next_cursor` 为本页首条 ID，耗尽时为 null。
