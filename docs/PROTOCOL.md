...
本机已激活账户分别保存经 Electron safeStorage 加密的激活码及后端返回的用户标识、用户名；仅一个账户持有运行中的会话，session JWT 只在内存。账户列表与切换留在主进程，不向渲染层或 Runner 暴露已保存的激活码；Runner 只可持工具所需本机连接信息。首次读取会话等待凭据恢复完成。会话过期保留本机账户缓存供重新激活，主动登出或身份变化时清理。切换先验证目标凭据，失败保留当前会话；成功后重建连接。过期会话的鉴权失败不得注销当前账户。桥接能力仍需独立准入。
