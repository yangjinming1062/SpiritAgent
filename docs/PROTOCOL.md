...
| POST /api/companion/avatar/{id}/fullbody/{kind}/prompt 与 /adopt（`kind` 取 `reference` / `front-2d` / `front-3d` / `back`）、POST /outfits/prompt 与 /adopt、/outfits/{id}/prompt 与 /adopt、/outfits/{id}/poses/{side}/prompt 与 /adopt、POST /room/prompt、/room/{id}/adopt 与 /room/{id}/discard | 自备图可选方案（与 AI 生成并列，不取代；LLM 自主生图链不参与）：prompt 端点按「文本身份锚定」变体组装完整提示词返回 `{prompt}`——不引用参考图输入、不含产品内部概念与供应商特有条款，供用户拿到任意外部工具生图；adopt 端点校验成品图后按对应生成端点「成功」的同一落库 / 状态 / 事件语义安装（种子图 persist 语义与元数据一致、衣橱采纳后为 draft 或状态回 draft 并发 `companion.outfit.updated`、房间行转 ready 并按同一着装指纹与激活策略广播 `companion.room.ready`）。房间 prompt 步骤即创建 `source=user_upload` 的 pending 行（brief/prompt 落库、不启动生图任务、不参与生成任务恢复，`BackdropResponse.source` 标记图片来源），由 adopt / discard 收敛。prompt 前置守卫与对应生成端点一致；自备图语境不区分 edit/regenerate（反馈恒并入重新生成语义）；单侧姿态采纳后抠图、关键点定位与闭眼帧仍由后端完成；身份经提示词文字传递，外部成品的外观一致性由用户把关 | Backend 生成服务 + Client 各生成点位自备图入口 + PIPELINE §1.1 / §6 + DESIGN §5.4 / §6.1 |
