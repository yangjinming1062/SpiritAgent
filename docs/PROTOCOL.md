...
| POST /api/companion/avatar/{avatar_id}/fullbody/front-2d | 以独立全身种子图为参考，按默认赛璐珞画风（自然站姿）与微调反馈生成/重绘 2D 正面全身图；全身种子图缺失时拒绝并提示先行生成。`mode="edit"` 微调上一版（反馈必填、无上一版拒绝），缺省 `regenerate` | Backend 生成 + Client 正面预览与微调 |
...
| GET/POST /api/companion/outfits 与 PATCH /policy、POST /{id}/regenerate、/{id}/confirm、PUT /{id}/activate、DELETE /{id} | 2D 换装衣柜：外观列表同时返回自主换装政策；每项的 `asset` 为该外观最新成功的 2D 包（复用 `Companion2DModelResponse` 的签名 manifest、图层 URL 与整包哈希），非就绪项或无成功包返回 null，读取不激活外观、不生成资产；草稿生成（着装描述 + 可选服装参考图，身份与身材恒为全身种子图主参考）/ 修改（draft 或 failed 可用：`mode="edit"` 微调上一版草稿（反馈必填、草稿过期拒绝），缺省 `regenerate` 按原描述与参考图全量重绘；成功后置 draft，重新确认才生成资产）/ 确认转正并触发 2D 切分（failed 可复用原立绘重试）/ 即时穿着 / 删除（穿着中与切分中拒绝）。政策取值为 `locked` / `llm_may_replace`，只门控夜间自主穿着与添置，不限制用户操作。生成走独立小时级频控，不设数量上限。微调模式要求供应商链具备图像编辑能力（gemini / grok），否则显式报错不回退 | Backend 生成管线 + Client 衣柜 + DESIGN §1.1 / §8 |
