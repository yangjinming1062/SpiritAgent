...
- 调用日志：携带调用标识的执行经 `utils/call_journal.py` 落盘参数指纹、执行阶段与可重取结果，终态先落盘再回复调用方，让中断后的重试能区分「已执行」与「从未开始」。执行权以原子建文件认领，并发同标识只有一个执行者；持有进程已死且无终态的记录转为待核对、不自动重放，终态不被迟到写入覆盖。日志不可写时放行执行，仅丢失恢复能力。契约见 [PROTOCOL §2.5](../docs/PROTOCOL.md)。
