...
interface ModelSeedWizardProps {
  avatarId: number
  /** 供应商是否按多图模式消费种子图（决定背面阶段是否存在）。 */
  supportsMultiview: boolean
  /** 确认按钮文案：默认按"首次切模型"表述；更新形象流程传入重建语义的文案。 */
  confirmLabel?: string
  /** 确认建模种子（或选择仅用正面图降级）后由调用方切换渲染模式并关闭向导。 */
  onConfirm: () => void
  onCancel: () => void
}
...
const STAGE_META: Record<
  ...
  {
    endpoint: string
    field: 'model_seed_front_url' | 'model_seed_back_url'
    title: string
    hint: string
    alt: string
    placeholder: string
...
> = {
  front: {
    endpoint: '/fullbody/model-front',
    field: 'model_seed_front_url',
    title: '升级模型：确认模型正面立绘',
    hint: '模型建模需要标准站姿（A-pose）的正面图，以你的形象头像为基准生成；不满意可微调重绘。',
    alt: '模型正面全身立绘',
    placeholder: '对模型正面立绘有微调要求？例如：姿势细节、服饰纹理…（可留空直接下一步）',
    genFail: '生成模型正面立绘失败，请稍后重试',
    loadFail: '正在生成模型正面立绘…'
  },
  back: {
    ...
    title: '升级模型：确认背面立绘',
    hint: '模型多视角建模以刚确认的模型正面立绘为基准补一张背面视图；不满意可微调重绘。',
    alt: '背面全身立绘',
...
// 模型升级前的种子图确认向导（DESIGN §5.5）：站姿要求与外观参考立绘不同（A-pose）——先以已确认的
// 外观参考为基准生成 A-pose 的模型正面立绘，多视角供应商再从它派生背面立绘，两者都
// 只在用户明确选择模型时生成。生成失败可降级为仅正面图提交。
export function ModelSeedWizard({
  avatarId,
  supportsMultiview,
  confirmLabel,
  onConfirm,
  onCancel
}: ModelSeedWizardProps): React.ReactElement {
  ...
  // 自备图参考图读本地缓存：front 用全身种子图，back 用本向导已确认的模型正面。
  ...
  const applySeedResponse = useCallback(
    async (
      key: Stage,
      res: { model_seed_front_url?: string | null; model_seed_back_url?: string | null } | undefined
    ): Promise<void> => {
  ...
  // 打开时水合已有模型种子（存量用户直接复用）；没有则停在空态，由用户点按显式触发——避免误触模型切换即消耗生图费用。
  useEffect(() => {
    const boot = async (): Promise<void> => {
      ...
      try {
        ...
        existingFront = res?.model_seed_front_url || null
        existingBack = res?.model_seed_back_url || null
    ...
    >
      ...
      >
        {current.previewUrl ? (
          ...
          <div className="flex flex-col items-center gap-3 px-4">
            <div className="text-xs text-muted">{stage === 'front' ? '尚未生成模型正面立绘' : '尚未生成背面立绘'}</div>
            <button className={BTN_PRIMARY} onClick={() => generate(stage, '')} type="button">
              {stage === 'front' ? '生成模型正面立绘' : '生成背面立绘'}
            </button>
          </div>
        )}
      </div>
      ...
      {current.failed && !current.previewUrl && (
        <div className="mt-3 flex items-center justify-between rounded-xl border border-line-hairline bg-surface-card px-3 py-2">
          <span className="text-muted">{stage === 'front' ? '模型正面立绘生成失败' : '背面立绘生成失败'}</span>
        ...
        referenceImages={
          stage === 'front'
            ...
            : stages.front.previewUrl
              ? [{ label: selfSourceDict.refs.seedModelFront, url: stages.front.previewUrl }]
              : undefined
        }
        title={`${stage === 'front' ? '模型正面立绘' : '背面立绘'} · 使用自己的图`}
      />
    </WizardModal>
  )
}
