...
export function CharacterCardSection({ avatarId }: { avatarId: number }): React.JSX.Element {
  ...
  return (
    <section className="space-y-3">
      ...
      <p className={HINT_TEXT}>{t.description}</p>
      ...
      {current && current.revision > 0 && (
        <div className="space-y-4 rounded-2xl border border-line-hairline p-4">
          {[
              ...
              {group.keys.map(key => (
                <div key={key}>
                  <div className="flex items-center justify-between gap-2">
                    ...
                    {!editing && current.overrides[key] != null && <span className={HINT_TEXT}>{t.edited}</span>}
                  </div>
                  {editing ? (
                    <textarea
                      ...
                      rows={3}
                      value={
                        Object.hasOwn(changes, key) ? (changes[key] ?? current.automatic[key]) : current.features[key]
                      }
                    />
                  ) : (
                    <p className="whitespace-pre-wrap text-sm text-body">{current.features[key] || t.unknown}</p>
                  )}
                </div>
              ))}
            </div>
      ...
      {editing && current && revision !== current.revision && <p className="text-xs text-amber-300/90">{t.conflict}</p>}
      ...
      {editing ? (
        <div className="flex flex-wrap gap-2">
          ...
          {current && revision !== current.revision && (
            <button className={BTN_SUBTLE} disabled={busy} onClick={() => setRevision(current.revision)} type="button">
              {t.merge}
            </button>
          )}
          <button
