...
(function (root, factory) {
  ...
  function normName(n) {
    ...
    if (n === 'leg_wear' || n === 'leg wear') n = 'legwear';
  ...
  function baseName(n) {
    ...
    if (raw === 'leg_wear' || raw === 'leg wear') return 'legwear';
  ...
  var SLOTS = {
    ...
    'leg-upper':   { depth: 0.88, group: 'body', order: 38 },
    'legwear':     { depth: 0.88, group: 'body', split: true, order: 40 },
    'bottomwear':  { depth: 0.88, group: 'body', order: 42 },
    'leg-lower':   { depth: 0.87, group: 'body', order: 44 },
    'foot':        { depth: 0.86, group: 'body', order: 46 },
    'footwear':    { depth: 0.86, group: 'body', split: true, order: 46 },
    'topwear':     { depth: 0.90, group: 'body', order: 50 },
    'arm-upper':   { depth: 0.88, group: 'body', order: 55 },
    'arm-lower':   { depth: 0.87, group: 'body', order: 58 },
    'hand':        { depth: 0.86, group: 'body', order: 60 },
    'handwear':    { depth: 0.86, group: 'body', split: true, order: 60 },
  ...
  function buildRig(psd, opts) {
    ...
    for (var ei = 0; ei < entries.length; ei++) {
      ...
      if (slot.split) {
        ...
        if (sideMatch) {
          ...
          if (!got) {
            var recFallback = makePart(e.name, e.layer, e.alpha, null, W, H, slot, z, null, null);
            if (recFallback) { parts.push(recFallback); z++; }
            warnings.push('"' + e.name + '" の左右分離に失敗（空レイヤー？または単一塊）');
          }
        }
    ...
    // 四肢与骨骼锚点估计
    var faceW = FACE.x1 - FACE.x0;
    var faceH = FACE.y1 - FACE.y0;
    var neckY = anchors.neckPivot.cy;
    var bodyH = Math.max(100, H - neckY);
    var shY = anchors.neckBottom + faceH * 0.15;
    anchors.shoulderL = { x: FACE.cx - faceW * 0.55, y: shY };
    anchors.shoulderR = { x: FACE.cx + faceW * 0.55, y: shY };

    var bwLayer = byName['bottomwear'] || byName['bottom_wear'];
    var bwBbox = bwLayer ? bboxOf(bwLayer.alpha, W, H, 8) : null;
    var hipY = bwBbox ? bwBbox.y0 + (bwBbox.y1 - bwBbox.y0) * 0.25 : (neckY + bodyH * 0.45);
    anchors.hip = { x: anchors.neckPivot.cx, y: hipY };
    var hipHalfW = Math.max(20, faceW * 0.45);
    anchors.hipL = { x: anchors.neckPivot.cx - hipHalfW, y: hipY };
    anchors.hipR = { x: anchors.neckPivot.cx + hipHalfW, y: hipY };

    var fwLayerL = sided['footwear|l'] || (byName['footwear_l'] && byName['footwear_l'].alpha) || (byName['footwear-l'] && byName['footwear-l'].alpha);
    var fwLayerR = sided['footwear|r'] || (byName['footwear_r'] && byName['footwear_r'].alpha) || (byName['footwear-r'] && byName['footwear-r'].alpha);
    var fwBboxL = fwLayerL ? bboxOf(fwLayerL, W, H, 8) : null;
    var fwBboxR = fwLayerR ? bboxOf(fwLayerR, W, H, 8) : null;
    var ankYL = fwBboxL ? fwBboxL.y0 + (fwBboxL.y1 - fwBboxL.y0) * 0.2 : (H - 30);
    var ankYR = fwBboxR ? fwBboxR.y0 + (fwBboxR.y1 - fwBboxR.y0) * 0.2 : (H - 30);
    anchors.ankleL = { x: anchors.hipL.x, y: ankYL };
    anchors.ankleR = { x: anchors.hipR.x, y: ankYR };
    anchors.kneeL = { x: anchors.hipL.x, y: (hipY + ankYL) * 0.5 };
    anchors.kneeR = { x: anchors.hipR.x, y: (hipY + ankYR) * 0.5 };
