...
function makeCanvas(container: HTMLElement): HTMLCanvasElement {
  ...
  canvas.className = 'companion-model-canvas'
  ...
  try {
    if (renderer instanceof WebGPURenderer && renderer.backend instanceof WebGPUBackend) {
      ...
      if (info) {
        ...
        if (desc) {
          log.info('model', `GPU adapter: ${desc}`)
        }
      }
    ...
    if (gl) {
      ...
      if (ext) {
        log.info('model', `GPU renderer: ${String(gl.getParameter(ext.UNMASKED_RENDERER_WEBGL))}`)
      }
    }
  } catch {
    /* 信息性日志——取不到就跳过 */
...
export class Engine {
  ...
  static async create(container: HTMLElement, opts: EngineOptions): Promise<Engine> {
    ...
    if (!forceClassic) {
      try {
        ...
        log.info('model', `Engine initialized with ${backendKind} backend (${width}x${height} @ ${dpr}x)`)
        ...
        log.warn('model', 'WebGPURenderer failed, falling back to classic WebGLRenderer:', gpuErr)
      }
    }
    ...
    try {
      ...
      log.info('model', `Engine initialized with classic-webgl backend (${width}x${height} @ ${dpr}x)`)
      logGpuAdapterInfo(renderer)
      ...
      glCanvas.remove()
      log.error('model', 'All model renderer backends failed:', glErr)
      throw new Error(
        `Failed to initialize any model renderer: ${glErr instanceof Error ? glErr.message : String(glErr)}`
      )
    }
  }
  ...
  private onWebGLContextLost = (): void => {
    ...
    log.warn('model', `${this.backendKind} context lost; waiting for restoration`)
  }
  ...
  private onWebGLContextRestored = (): void => {
    ...
    try {
      ...
      log.info('model', 'classic-webgl context restored')
    } catch (error) {
  ...
  private requestRecovery(reason: string): void {
    ...
    this.invalidateHitmap()
    this.hitRefresh = null
    log.warn('model', `${reason}; rebuilding engine`)
  ...
  private async refreshHitmap(): Promise<SilhouetteHitmap | null> {
    ...
    try {
      ...
      if (!this.disposed) {
        log.warn('model', 'silhouette hitmap readback failed:', err)
      }
