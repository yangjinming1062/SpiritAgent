...
interface ModelStageModelResponse {
  id: number
  asset_url: string | null
  provider: string
  species: string
...
// 引擎解析失败回退到程序化蛋（createProcedural）时置为 true；sprite-window 的渲染级联会
// 按形象就绪情况选择视频或模型渲染层，两级皆不可用才停在程序化蛋（DESIGN §1.2）。
...
export async function rebuildModel(): Promise<void> {
  ...
  const result = await authedApi<ModelStageModelResponse>({
...
export function setModelInfo(next: Partial<ModelInfo>): void {
  ...
  if (next.status !== undefined && next.status !== 'succeeded') {
    ...
    // 否则 Partial 合并会让上一帧的 succeeded 字节残留在 atom 里，模型渲染器会拿旧 asset_url
...
export async function hydrateModel(): Promise<void> {
  const result = await authedApi<ModelStageModelResponse | null>({
    path: '/api/companion/model'
  })
  ...
  if (!result.ok) {
    ...
    // hydrateModel 只读不写：模型生成由 confirm-front 成功后显式触发，避免 onboarding 中段误启动
    return
  }
