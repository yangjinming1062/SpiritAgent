...
export function ModelStage(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    const ready = Engine.create(container, {
      ...
      // 拖拽速度监听器：拖动窗口时让模型物理跟随倾斜 / 摆动
      const unsubDragVelocity = $dragVelocity.listen(vel => {
        eng.character.setDragVelocity(vel.vx, vel.vy)
      })

      // 移动肢体动画（DESIGN §3.3）：walk/walk_fast 移动时播放 walk clip
      // （fly 无专属 clip，维持状态 clip）。
    ...
    void ready.catch(err => {
      // 整条降级链都失败了 —— 完全拿不到 GPU 上下文。置为失败态交给根渲染兜底
      // （程序化蛋 / 状态提示，见 DESIGN §1.2 级联）；这里置为
      // settled，调度器可以歇下来。
      if (!cancelled) {
        log.error('companion-model', 'engine init failed:', err)
    ...
    void (async () => {
      ...
      if (url) {
        try {
          ...
          log.warn('companion-model', 'GLB fetch failed:', err)
        }
      }
      ...
      if (url && !bytes && identity && loadedIdentityRef.current === identity) {
        log.warn('companion-model', 'GLB fetch failed; preserving currently rendered character model')
      ...
      try {
        ...
        // 因此只有 GLB 真正解析完才会切到模型（不会出现蛋形态闪现）。
        ...
        log.error('companion-model', 'loadCharacter failed:', err)
        $glbLoadFailed.set(true)
        $modelLoadSettled.set(true)
  ...
  return (
    <div
      className="companion-model-wrapper"
      ref={containerRef}
