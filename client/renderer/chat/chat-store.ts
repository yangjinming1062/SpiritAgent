  ...
  for (const m of messages) {
    ...
    for (const [index, segment] of segments.entries()) {
      ...
      const cachedVoiceDuration =
        m.role === 'assistant' && segment ? getCachedVoiceDuration(segment, m.speech_style) : undefined
...
export function finalizeAssistantMessage(
  ...
  const shouldSynth = (options?.synthesize ?? true) && isLivingVoiceBarActive() && Boolean(speechText(finalStr))
  ...
  $chatMessageBodies.setKey(lastItem.id, {
    ...
    streaming: false,
    toolName: null,
    voiceDuration: body.voiceDuration ?? (finalStr ? getCachedVoiceDuration(finalStr, speechStyle) : undefined),
