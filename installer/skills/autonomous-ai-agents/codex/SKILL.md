...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the SpiritAgent terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For SpiritAgent itself, `model.provider: openai-codex` uses SpiritAgent-managed Codex
OAuth from `${SPIRITAGENT_HOME:-~/.spiritagent}/auth.json` after `spiritagent auth add openai-codex`. For the
...
## SpiritAgent Gateway Caveat

When invoking the Codex CLI from a SpiritAgent gateway/service context (for example,
