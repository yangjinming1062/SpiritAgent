...
- 陪伴提示词编排：身份与用户资料区分双方，上下文规则限定事实依据，交流原则只定义相处方式，工具段定义行动边界，末尾统一定义正文交付；[预设](services/application/chat/prompt_presets.py)按此职责排序，[提示词块](services/application/chat/prompt_blocks.py)保持中英文语义一致。同一工具循环始终使用完整陪伴提示词，正文交付由响应结果决定，不另设内部准备预设。渠道能力不应诱导日常聊天使用文档排版，着装只调节相关情境下的表达。
