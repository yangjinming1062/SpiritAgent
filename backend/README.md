...
- 供应商回退边界：`execute_with_fallback` 的 `on_first_chunk` 哨兵防止 mid-stream 切换供应商——一旦收到首个供应商流事件就锁死当前供应商（包括缓冲期），避免同一回合混合两个模型的输出。不完整终态可在同一供应商上有界重试，正文交付与重试边界见 [PROTOCOL §1.4](../docs/PROTOCOL.md)。失败、取消或拒绝异常工具输出时逐层关闭响应流，及时释放连接。
...
- 气泡边界：陪伴预设的空行与专用分隔行共用流式切分，跨 chunk 暂留可能属于较长分隔符的前缀，避免把分隔线泄漏为正文；生活空间陪伴回合先静默准备工具，再禁用工具按完整气泡发送正式答复；工作台正文实时推送；工具循环的分流、上下文与 TTS 边界见 [PROTOCOL §1.4](../docs/PROTOCOL.md)。专业预设不按空行切分。
