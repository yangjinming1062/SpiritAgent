...
隔离边界见 [ARCHITECTURE §6.5](../docs/ARCHITECTURE.md#65-记忆隔离边界)。[记忆存储入口](services/domains/memory/memory_store.py)集中写入、来源校验、槽位更新与回填；应用层和适配层不直接修改记忆。来源会话与消息必须属于调用域，普通 ID 操作同样带作用域条件。

预设列非空且无默认值，漏传必须失败；唯一槽位按用户、预设和分类约束，普通召回分类允许多条。向量候选通过物化子查询先限定同域，再精确计算距离，避免共享近似索引先截断其他域候选而影响召回。

内容改变递增版本；延迟向量回填同时匹配作用域、ID、内容和版本，过期结果不重建记录。反思槽位与归并采用来源快照条件更新，避免覆盖并发人工修改或删除。

回合后提取按目标选择提示词：陪伴保留关系与情感线索，工作预设只提取事实、偏好、约束和工具经验，避免共享辅助提示词把陪伴语义引入工作记忆。

模型历史只读取 `Message.id > context_after_message_id`，摘要加载与分支复制遵守同一水位。夜间日志按用户、预设和日期去重，避免某域成功后跳过其他域。

备份恢复重映射上下文水位、父会话及来源引用，合并槽位也按预设区分；带隔离水位的同一系统会话拒绝合并历史，避免重新暴露旧上下文。备份格式和管理维护边界见 [PROTOCOL](../docs/PROTOCOL.md#预设记忆与学习作用域)。

### 夜间批处理

- 活动规划：休息窗口内统一执行画像、记忆整理、自主规划与日记；白天阈值整理与夜间整理按用户和预设互斥，LLM 返回后仅在源记忆快照未被用户编辑或删除时以短事务替换。规划使用高推理档和当时可用的安全能力目录，输入角色、衣橱、房间、供应商、画像、近期历史与政策，允许小组合或空计划。无当日消息仍可基于生日等长期记忆规划；无新互动的画像 / 日记步骤跳过，不能伪造经历。
