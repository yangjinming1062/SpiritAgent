...
- 内部结构化提示词：稳定任务、事实边界与输出契约放在 `instructions`，运行时人设、对话、记忆、参考资料和可选枚举以 JSON 数据输入；结构化结果由代码解析，并按约定字段与枚举归一化或校验。视觉生成围绕当前成图目标组织主体、参考职责、构图与光线；独立全身肖像以正向描述明确体态与气质，不携带建模流程说明或无关排除项。伙伴视觉链的双参考图固定图 1 为身份锚点、图 2 只承担当前任务指定的次级视觉参考；拼图位置与单幅成图要求由生图适配层统一说明。
