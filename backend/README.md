...
- 陪伴提示词编排：身份与用户资料区分双方，上下文规则限定事实依据，交流原则只定义相处方式，工具段定义行动边界，末尾统一定义正文交付；[预设](services/application/chat/prompt_presets.py)按此职责排序，[提示词块](services/application/chat/prompt_blocks.py)保持中英文语义一致。准备阶段共享事实背景与工具规则，但不装配交流风格、正文格式和语音协议，避免同时要求内部就绪与角色聊天；正式回复再装配完整表达规则。渠道能力不应诱导日常聊天使用文档排版，着装只调节相关情境下的表达。
- 专业提示词编排：四个专业预设的双语头部只定义职业判断与交付标准，共享模板统一装配协作原则、本域资料与记忆、按能力启用的工具规则及渠道环境。工具纪律尊重用户开关，技能维护取决于复用价值而非调用次数；不重复注入陪伴能力介绍或要求固定输出结构。职业体验边界见 [DESIGN §8](../docs/DESIGN.md#8-多预设并列的系统对话)。
