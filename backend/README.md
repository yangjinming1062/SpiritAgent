...
- **域间例外**（脚本例外表的权威）：各域可单向导入 conversation——它是一切回合与叙事的会话底座且零反向依赖；companion 使用 memory 公共入口读取陪伴记忆与记录交互，journal 使用 memory 的时区解析。业务域不反向依赖应用流程；首张房间图由 API 在全身立绘确认成功后调度。
...
- **显式注册**（`bootstrap/registrations.py`）：LLM 供应商、图生 3D 供应商、LLM 工具与 memory 工具 schema、渠道适配器、内部事件处理器（`companion.turn.request` → `execute_companion_turn`）、域钩子（调度变更时撤销待兑现意图）。新增供应商/工具/渠道 = 实现类 + 装配层加一行注册；遗漏会在能力链解析时显式抛 `LookupError` 而非静默缺能力。注册表覆盖式幂等，重复调用安全。
...
- 提示词装配：设计、编写、修改与验证遵循 [RULES](../RULES.md#提示词设计与修改规范)。内部结构化推理通过 `instructions` 承载任务契约，运行时资料以 JSON 输入，结构化结果由代码解析并按约定字段与枚举归一化或校验。独立全身肖像的视觉任务围绕体态与气质组织；伙伴视觉链的双参考图固定图 1 为身份锚点、图 2 只承担当前任务指定的次级视觉参考，拼图位置与单幅成图要求由生图适配层统一说明。
...
- 设备执行资格由客户端 `tools.sync` 汇总发布：Runner 断开/停止时客户端发空列表即时撤销注册表（见 [PROTOCOL §1.2](../docs/PROTOCOL.md)），断线宽限窗内不再派发本机工具；宽限期只服务会话恢复，不能替代可执行判断。IM 对端白名单没有逐次授权层，批准对端即授予本机操作能力——是否细分授权属产品决策。
