...
class CompanionScene(ModelBase, TimestampMixin):
    ...
    # 供应商返回后先保存地址；转存重试与描述重试均不再提交生图。
    result_url: Mapped[str] = mapped_column(Text, default="", server_default=text("''"))
    ...
    identity_best_path: Mapped[str] = mapped_column(String(2048), default="", server_default=text("''"))
