...
from services.companion import update_mood_from_companion_turn
from services.conversation import DEFAULT_PRESET_ID, SPECIAL_KIND
...
def _track_background_task(user_id: int, task: asyncio.Task) -> None:
...
async def _persist_assistant_no_tool_turn(
    ...
    *,
    ...
    if persist and run_post_turn_tasks and bg_review.lower() == BACKGROUND_REVIEW_DEFAULT:
        review_task = asyncio.create_task(
            run_background_memory_review(user_id, llm_config, copy_responses_context(context)),
        )
        if track_task:
            track_task(review_task)
        else:
            _track_background_task(user_id, review_task)
    ...
    if (
        ...
        if track_task:
            track_task(mood_task)
        else:
            _track_background_task(user_id, mood_task)
