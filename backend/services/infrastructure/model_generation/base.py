...
@dataclass(frozen=True)
class CharacterModelPollResult:
...
class CharacterModelError(Exception):
    """图生模型服务及供应商的基础异常。
...
class CharacterModelProvider(ABC):
    """图生模型供应商抽象基类；能力 ClassVars 控制 ``pipeline.run_capability_chain`` 中的供应商专属流水线步骤，编排侧在调用可选的 rig / animate_bind 方法前先检查它们。
