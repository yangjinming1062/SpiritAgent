import asyncio
import base64
import io
import zipfile
from pathlib import Path

from components import SETTINGS, log_paid_call

from ...base import (
    CharacterModelAsset,
    CharacterModelError,
    CharacterModelJob,
    CharacterModelPollResult,
    CharacterModelProvider,
)
from . import client
from .client import HunyuanApiError

_MAX_IMAGE_BYTES = 10 * 1024 * 1024
_ALLOWED_SUFFIXES = {".png", ".jpg", ".jpeg", ".webp"}
# 供应商产物 zip 压缩比不可控，解压前按声明大小封顶，防 zip 炸弹
_MAX_GLB_EXTRACT_BYTES = 512 * 1024 * 1024

# TokenHub 任务状态 → CharacterModelPollResult.status；未知值继续轮询。
_STATUS_MAP: dict[str, str] = {
    "queued": "queued",
    "pending": "queued",
    "in_progress": "in_progress",
    "running": "in_progress",
    "completed": "completed",
    "succeeded": "completed",
    "failed": "failed",
}


class HunyuanCharacterModelProvider(CharacterModelProvider):
    """腾讯混元生 3D（TokenHub OpenAI 兼容接入），支持单图与多视图。当前无云端 rig / animate-bind 端点——客户端可消费产物但仅显示静态模型(无法驱动骨骼动画)。"""

    provider_name = "hunyuan"
    SUPPORTS_RIGGING = False
    SUPPORTS_MULTIVIEW = True
    SUPPORTS_ANIMATE_BIND = False
    DEFAULT_MODEL = "hy-3d-3.1"

    @property
    def _model(self) -> str:
        return getattr(SETTINGS, "hunyuan_model_version", "") or self.DEFAULT_MODEL

    async def _read_b64(self, path: Path) -> str:
        if path.suffix.lower() not in _ALLOWED_SUFFIXES:
            raise CharacterModelError(
                f"种子图格式不支持（{path.suffix}，允许 jpg/png/webp）",
                provider=self.provider_name,
            )
        image_bytes = await asyncio.to_thread(path.read_bytes)
        if len(image_bytes) > _MAX_IMAGE_BYTES:
            raise CharacterModelError(f"种子图超过 10MB 上限（{len(image_bytes)} bytes）", provider=self.provider_name)
        return base64.b64encode(image_bytes).decode("ascii")

    async def create_image_to_model(
        self,
        image_path: Path,
        *,
        multiview_paths: dict[str, Path] | None = None,
    ) -> CharacterModelJob:
        try:
            auxiliary_paths = {key: path for key, path in (multiview_paths or {}).items() if key != "front"}
            front_b64 = await self._read_b64(image_path)
            if self.SUPPORTS_MULTIVIEW and auxiliary_paths:
                auxiliary_images = {key: await self._read_b64(path) for key, path in auxiliary_paths.items()}
                task_id = await client.create_image_to_model(
                    front_b64,
                    multiview_images=auxiliary_images,
                    **client.hunyuan_common_kwargs_from_settings(),
                )
            else:
                task_id = await client.create_image_to_model(front_b64, **client.hunyuan_common_kwargs_from_settings())
            return CharacterModelJob(job_id=task_id)
        except HunyuanApiError as exc:
            raise CharacterModelError(str(exc), provider=self.provider_name, model=self._model) from exc
        except ValueError as exc:
            raise CharacterModelError(str(exc), provider=self.provider_name, model=self._model) from exc

    async def poll(self, job: CharacterModelJob) -> CharacterModelPollResult:
        try:
            body = await client.get_task(job.job_id, model=self._model)
        except HunyuanApiError as exc:
            raise CharacterModelError(str(exc), provider=self.provider_name, model=self._model) from exc

        status = _STATUS_MAP.get(str(body.get("status", "")).lower(), "in_progress")
        if status == "completed":
            assets = tuple(
                CharacterModelAsset(
                    kind=str(item.get("type") or "").lower(),
                    url=str(item.get("url") or ""),
                    preview_image_url=item.get("preview_image_url"),
                )
                for item in body.get("data") or []
                if isinstance(item, dict) and item.get("url")
            )
            log_paid_call(
                self.provider_name,
                "model_generation_result",
                task_id=job.job_id,
                urls=[a.url for a in assets],
                level="debug",
            )
            return CharacterModelPollResult(status="completed", progress=100, assets=assets)
        if status == "failed":
            return CharacterModelPollResult(
                status="failed",
                error=str(body.get("error") or body.get("message") or body)[:500],
            )
        return CharacterModelPollResult(status=status, progress=0)

    async def download(self, result: CharacterModelPollResult, dest_dir: Path) -> Path:
        glb_urls = [a.url for a in result.assets if a.kind.lower() == "glb"]
        any_urls = [a.url for a in result.assets if a.url]
        if not any_urls:
            raise CharacterModelError("hunyuan 任务完成但未返回模型下载地址", provider=self.provider_name)
        dest_dir.mkdir(parents=True, exist_ok=True)
        raw = await client.download_model(glb_urls[0] if glb_urls else any_urls[0])
        if raw[:4] == b"PK\x03\x04":
            glb_path = await asyncio.to_thread(_extract_glb_from_zip, raw, dest_dir)
            if glb_path is None:
                raise CharacterModelError("hunyuan 产物 zip 内未找到 GLB", provider=self.provider_name)
            return glb_path
        dest = dest_dir / "hunyuan_model.glb"
        await asyncio.to_thread(dest.write_bytes, raw)
        return dest


def _extract_glb_from_zip(raw: bytes, dest_dir: Path) -> Path | None:
    with zipfile.ZipFile(io.BytesIO(raw)) as zf:
        for name in zf.namelist():
            if name.lower().endswith(".glb"):
                if zf.getinfo(name).file_size > _MAX_GLB_EXTRACT_BYTES:
                    raise CharacterModelError(
                        f"hunyuan 产物 zip 内 GLB 超过解压上限（{zf.getinfo(name).file_size} bytes）",
                        provider="hunyuan",
                    )
                out = dest_dir / Path(name).name
                out.write_bytes(zf.read(name))
                return out
    return None


__all__ = ["HunyuanApiError", "HunyuanCharacterModelProvider"]
