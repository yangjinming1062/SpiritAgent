...
def _apply_runtime_side_effects(changed_keys: set[str]) -> None:
    ...
    if "rate_limit_enabled" in changed_keys:
        try:
            from services.rate_limit import limiter

            limiter.enabled = bool(SETTINGS.rate_limit_enabled)
            logger.info("Rate limiter enabled updated to %s", limiter.enabled)
        except Exception:
            logger.warning("Failed to update limiter status", exc_info=True)
    ...
    if changed_keys & llm_http_keys:
        try:
            from services.llm import rotate_http_clients
