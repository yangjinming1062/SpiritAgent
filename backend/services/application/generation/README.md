...
头像生成、全身生成、选择和确认共用用户级锁。模型配置在短会话内读取，等待供应商时不持有读事务；写入前核对身份、状态和源路径，防止迟到覆盖。状态与刷新事件同事务提交；被替换的未采纳候选清理图片，已采纳的旧全身图保留给历史任务。
...
默认外观、视频启动标记与启动错误使用 ORM 显式字段，启动标记与首个视频包同事务写入。外观描述任务按用户及外观合并在途请求，停机时取消并等待退出。衣柜命名读取实际采纳图，先视觉转写再生成名称与描述；失败保留原名且不阻塞资产就绪，回写核对图片路径。
