...
from services.infrastructure.model_generation import (
    CharacterModelAsset,
    CharacterModelError,
    CharacterModelJob,
...
class _PipelineState:
    ...
    assets: tuple[CharacterModelAsset, ...]


def _resolve_model_provider(name: str | None) -> CharacterModelProvider:
    """只按显式指定解析——商业供应商之间绝不互相故障转移。"""
    try:
        return resolve_provider(name)
    except (CharacterModelError, LookupError) as exc:
        ...
        superseded = (
            await db.execute(
                ...
                .where(
                    ...
                    CompanionModel.status.in_(IN_FLIGHT_STATUSES),
                )
                .limit(1),
            )
...
async def _persist_clip_map(model_id: int, clip_map: dict[str, str]) -> None:
    ...
    async with SESSION_LOCAL() as db:
        await db.execute(
            update(CompanionModel)
            .where(CompanionModel.id == model_id)
...
async def _refresh_download_urls(
    ...
    *,
    ...
    model_id: int,
    task_id: str,
) -> list[CharacterModelAsset]:
    """仅通过查询已持久化的供应商任务刷新过期签名地址，绝不重新提交生成。"""
    result = await provider.poll(CharacterModelJob(job_id=task_id))
...
async def _cas_model_status(model_id: int, *, from_statuses: tuple[str, ...], to_status: str) -> bool:
    ...
    async with SESSION_LOCAL() as db:
        result = await db.execute(
            update(CompanionModel)
            .where(CompanionModel.id == model_id, CompanionModel.status.in_(from_statuses))
...
async def _download_with_retry(
    ...
    *,
    ...
    model_id: int,
    task_id: str | None,
    assets: list[CharacterModelAsset],
    dest_dir: Path,
) -> Path:
    ...
    for attempt in range(1, _DOWNLOAD_ATTEMPTS + 1):
        try:
            return await provider.download(CharacterModelPollResult(status="completed", assets=tuple(assets)), dest_dir)
...
async def _poll_with_progress(
    ...
    deadline = time.monotonic() + SETTINGS.model_generation_max_poll_seconds
    ...
    while True:
        ...
        if remaining <= 0:
            await _drain()
            raise ModelGenerationError(f"模型生成超时({SETTINGS.model_generation_max_poll_seconds:.0f}s)")
        ...
        if result.status == "failed":
            await _drain()
            raise ModelGenerationError(f"模型生成任务失败: {result.error or stage}")
        ...
        raw = result.progress or min(
            100,
            int(100 * (time.monotonic() - started) / SETTINGS.model_generation_max_poll_seconds),
        )
        ...
        sleep_for = backoff_for_poll(
            attempt,
            base_interval=SETTINGS.model_generation_poll_interval_seconds,
            max_interval=SETTINGS.model_generation_poll_backoff_max_seconds,
...
async def _maybe_apply_capability(
    ...
    *,
    ...
    rig_type: str,
    phase: str,
    submit: Callable[[], Awaitable[CharacterModelJob]],
    ...
    try:
        ...
        new_job: CharacterModelJob = await submit()
...
async def run_capability_chain(
    *,
    ...
    try:
        ...
        await _emit_model_failed(user_id, "暂未配置模型生成供应商，请联系管理员")
    ...
    try:
        if current_task_id is None:
            ...
            gen_result = await _poll_with_progress(
                provider,
                CharacterModelJob(job_id=job.job_id),
                user_id,
                "generating",
                10,
                50,
            ...
            job = CharacterModelJob(job_id=current_task_id)
        ...
        await _emit_model_failed(user_id, "模型生成失败，请稍后重试")
        await _mark_generation_failed(model_id, "模型生成失败，请稍后重试")
        return
    ...
    try:
        ...
        if not activated:
            logger.info(
                "model generation superseded by a newer run; asset saved without activating",
                extra={"user_id": user_id, "model_id": model_id},
            )
            return
        ...
        await _emit_model_failed(user_id, "模型生成失败，可重试下载", retry_download=True, model_id=model_id)
...
def _launch_pipeline_task(*, model_id: int, user_id: int, **kwargs: object) -> None:
    ...
    async def _runner() -> None:
        try:
            ...
            await _emit_model_failed(user_id, "模型生成失败，请重试", model_id=model_id)
        finally:
            _inflight_tasks.pop(model_id, None)

    task = asyncio.create_task(_runner(), name=f"companion.model.{user_id}.{model_id}")
...
async def recover_stuck_model_generations() -> None:
    ...
    async with SESSION_LOCAL() as db:
        await db.execute(
            update(CompanionModel)
            .where(CompanionModel.status == "generating")
...
async def resume_inflight_pipelines() -> None:
    ...
    async with SESSION_LOCAL() as db:
        rows = (
            (await db.execute(select(CompanionModel).where(CompanionModel.status.in_(IN_FLIGHT_STATUSES))))
            .scalars()
            .all()
        )
    for row in rows:
...
async def _probe_paid_failure(row: CompanionModel) -> _ResumeOutcome:
    ...
    try:
        ...
        result = await provider.poll(CharacterModelJob(job_id=row.provider_task_id))
    except Exception:
        logger.warning(
