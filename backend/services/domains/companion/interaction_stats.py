...
def _format_content(counters: _DailyCounters) -> str:
    ...
    return f"{counters.date}: chat_turns={counters.chat_turn}; peak={peak_str}; hour_counts={sorted_buckets}"
...
async def _upsert_memory(user_id: int, counters: _DailyCounters) -> None:
    ...
    logger.info(
        ...
        extra={"user_id": user_id, "date": counters.date, "chat_turn": counters.chat_turn},
    )
...
async def record_interaction(user_id: int, kind: str, hour: int) -> dict:
    ...
    threshold_met = counters.chat_turn >= STATS_THRESHOLD
