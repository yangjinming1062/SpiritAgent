...
# 扶边姿态：遮挡布局与背景策略由 mesh2d/poses.py 装配。
PEEK_POSE_PROMPT_TEMPLATE = """将参考图中的角色绘制成从竖直遮挡物后探头、双手轻扶边缘看向观众的姿态。图片用于后续遮挡合成，构图以遮挡后的观看效果为准。

使用 1:1 画布，以画面正中央的竖直线作为假想遮挡边界。按观众看到的画面划分：{layout.occluded_half}是遮挡区，{layout.visible_half}是露出区。展示时遮挡区会被盖住，露出区保留可见。
遮挡后的效果：完整面部、探出的肩部和部分上胸、两只扶边的手都在露出区清晰可见，形成自然的探头动作。

角色的骨盆与双腿留在遮挡区自然站稳，腰背侧弯带动肩胸跨过边界；头颈顺着上身的探出趋势，头部轻轻朝露出区倾斜，整张脸越过边界。面向观众，双眼睁开，神情温和好奇。
两只手一上一下轻扶同一条边界，上手约在下颌高度，下手约在上胸高度。手掌、手腕与自然弯曲的双臂位于露出区，指尖轻扣边缘；像从遮挡物后探头时扶住边缘，而不是伸手向外推。

参考图只确定角色外观：保留面容、物种、发型发色、身体比例、服装配饰、配色和画风，保留原有不对称细节，不镜像角色；姿态与构图按上述要求重绘。
输出尚未加遮挡的完整角色素材：遮挡区内的身体也要完整绘制，从头顶到双脚、鞋履及原有身体附属结构全部入画，四周留有余量，上下各约 8%。不实际遮住或裁掉身体，不画遮挡物或边界线。
{background_prompt}
不添加文字或水印。"""
