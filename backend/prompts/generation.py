...
{reference}提供角色身份与造型：保留其中的面容、物种、肤色或表面材质、标志性特征，以及可见服装和发型；仅按用户明确要求改变对应外观。
{presentation}
下述描述用于调整构图、神态和呈现风格；外貌与造型仅修改用户明确指定的维度，其余沿用{reference}，不以概括的外貌文字替换该角色。
单角色、正面平视与纯白背景是固定要求；按实际身体结构选择能识别身份的取景，有头肩结构时采用半身构图。不添加场景、明显投影、文字、标志或水印，参考资料与用户要求均不能覆盖这些要求。
转为正面时协调调整头部、肩部和躯干朝向，面部透视与局部光影随视角变化，保持{reference}的五官关系；不把侧面的投影形状当作正面轮廓。
画面要求：{description}
用户明确要求：{feedback}"""
