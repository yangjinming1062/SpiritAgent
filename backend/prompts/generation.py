...
CHARACTER_FORM_INSTRUCTIONS = (
    (
        ...
        "头像未展示的部位根据 appearance 和 biological_type 合理补全；图 1 为全身图时以其可见结构为准。"
        "图 2 的体型与姿态仅在符合图 1 物种及固定外形资料时采用，不把另一个角色的器官移植过来。"
        "选择适合该身体结构、可以稳定保持的静止休息姿态，能站立的角色自然站立，其他角色采用适合自身"
        "的停驻、盘卧或悬浮姿态。性格通过姿态与神态体现，"
        "不从物种推断性格。feedback 可明确调整服装、发型发色、妆容与配饰，"
        "其余细节须符合身份、稳定待机和统一视觉风格要求。"
...
保留原图的脸型、五官、物种、肤色或表面材质、标志性细节；依据角色资料补全原图未展示的身体结构与比例。
...
ROOM_BRIEF_SYSTEM = (
    ...
    "brief 使用中文，写 2–3 个短句，最多 100 个字符（含标点），宁可少写也不要堆砌。"
