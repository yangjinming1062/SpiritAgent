"""生成链提示词文本：半身头像改写、全身画风词典、姿态短语与扶边姿态模板、换装/自备图身份条款与改写句式、
...
# 扶边姿态：方向字段与背景策略由 mesh2d/poses.py 装配。
PEEK_POSE_PROMPT_TEMPLATE = """修改参考图，将图中角色调整为从画面{f.source_edge}侧向{f.lean_dir}探身、双手轻扶边缘的姿势。
保持原图角色的脸型、五官、物种、发型发色、身体比例、服装配饰、配色和画风，包括身体与服装的不对称细节；不要镜像角色。只调整姿态、构图和下述背景。

使用 1:1 画布，左右均以观察者视角为准。画布横向正中央有一条假想的竖直边缘，作为探身和双手扶握的共同基准；不画出线条或任何遮挡物。
骨盆、双腿和双脚主要留在画面{f.body_half}稳定支撑；腰背自然向{f.lean_dir}侧弯，带动胸廓和肩部越过接触线，使完整面部、靠近探出方向的肩部及上胸重心进入{f.peek_half}。接触线位于探出的脸胸与留在来源侧的骨盆之间，形成从边缘后探头看向观众的姿态。不要仅歪头、向前鞠躬或将全身倾斜。
头部自然跟随肩部，面向观众，双眼睁开，表情温和好奇。双臂放松屈肘，双肘位于{f.peek_half}，前臂从{f.lean_dir}向{f.source_edge}伸回接触线，双手在脸部的{f.source_edge}侧一上一下轻扣同一条边缘；上手约在下颌高度，下手约在上胸高度。不要把双手伸到脸部的{f.lean_dir}侧或做向外推的动作。手指轻弯，手腕与手臂连接自然，不增加或扭曲肢体及手指。

完整保留角色，从头顶到双脚、鞋履及原有身体附属结构均入画，不裁切或遮挡。上下各留约 8% 空间，左右轮廓不贴边。
{background_prompt}
不添加文字或水印。"""

POSE_TRANSPARENT_BACKGROUND = """输出带真实 Alpha 通道的透明背景 PNG，角色以外区域全部透明，不画棋盘格或背景投影。
保留发丝、平滑轮廓及原有半透明材质，边缘无残留底色。"""

POSE_CHROMA_BACKGROUND_TEMPLATE = (
    """背景使用{color}，纯色均匀铺满至画布四边，无纹理、渐变、棋盘格、投影或晕影；角色光线不溢染背景。"""
)
...
BLINK_EDIT_PROMPT = (
    "修改输入图片，将角色双眼调整为自然闭合的眨眼状态，保留原有轻松表情。"
    "只改动眼睑及紧邻的眼部像素，闭眼曲线沿用原眼位、面部透视与画风。"
    "其余区域的外观与位置不变；保持原图尺寸、构图和背景，输出完整图片，使两帧叠放时眼部准确对齐。"
)
...
EDIT_PRESERVE_OUTFIT = (
    f"{EDIT_PRESERVE_2D_FRONT}。本次只修改服装、发型发色和配饰中明确指定的部分，未提及的造型细节保持不变"
)
