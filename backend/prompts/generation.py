"""生成链提示词文本：半身头像改写、全身画风词典与姿态短语、换装/自备图身份条款与改写句式、
编辑保持条款、服装参考图转写、房间背景规则与光线词典、审核合规改写、衣柜描述与房间陈设生成。
确定性拼接器与 LLM 调用在 services 各层。

IdentityAnchor（全身/换装提示词身份锚定变体）在此集中定义，供 prompt_engineer 与
application/generation 共用，避免应用层与基础设施层各维护一份 Literal。
...
# 全身/换装提示词的身份锚定变体：reference=AI 生图路径（种子参考图随请求传入）；
# reference-self-source=自备图路径（客户端本地缓存种子参考图，提示词以改写句式引用该图）。
# onboarding 完成后种子图恒在，两条路径都必须带参考图，无纯文字变体。
IdentityAnchor = Literal["reference", "reference-self-source"]
...
# 自备图：客户端本地缓存种子参考图，提示词以改写句式开头，同时适配外部工具图生图与纯文生图。
SELF_SOURCE_REFERENCE_CLAUSE = (
    "以提供的参考图为身份锚点，画同一角色的脸型、五官、体型、物种、性别与标志性身体特征，不得替换成其他人物。"
)
SELF_SOURCE_REWRITE_LEAD = "把参考图中的角色改成{target}。"
